...
def FilterObservation(
    ...
    if isinstance(env, gym.vector.VectorEnv):
        return _VectorFilterObservation(env)
    else:
        return _FilterObservation(env)


class _FilterObservation(gym.ObservationWrapper):
...
class _VectorFilterObservation(gym.vector.VectorObservationWrapper):
    """Filter Dict observation space by the keys.
    ...
    Example:
        ...
        ...     gym.vector.make('CartPole-v1', 8), lambda obs: {'obs': obs, 'time': 0}
        ... )
        >>> envs.observation_space = gym.spaces.Dict(obs=env.observation_space, time=gym.spaces.MultiDiscrete(np.full((8,), 1)))
        >>> envs.reset()
        {'obs': array([-0.00067088, -0.01860439,  0.04772898, -0.01911527], dtype=float32), 'time': array[0, 0, 0, 0, 0, 0, 0, 0]}
        >>> envs = FilterObservation(envs, filter_keys=['time'])
        >>> envs.reset()
        {'obs': array([ 0.04560107,  0.04466959, -0.0328232 , -0.02367178], dtype=float32)}
        >>> envs.step(np.zeros(8))
        ({'obs': array([ 0.04649447, -0.14996664, -0.03329664,  0.25847703], dtype=float32)}, 1.0, False, {})
    """

    def __init__(self, env: gym.vector.VectorEnv, filter_keys: Sequence[str] = None):
        """A wrapper that filters dictionary observations by their keys.
        ...
        wrapped_observation_space = env.single_observation_space
        if not isinstance(wrapped_observation_space, spaces.Dict):
            raise ValueError(
                f"FilterObservationWrapper is only usable with dict observations, "
                f"environment observation space is {type(wrapped_observation_space)}"
        ...
        self.single_observation_space = type(wrapped_observation_space)(
            [
                (name, copy.deepcopy(space))
                for name, space in wrapped_observation_space.spaces.items()
                if name in filter_keys
